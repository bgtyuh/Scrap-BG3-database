...
image_folder = 'data/images/cloak_images'
...
conn = sqlite3.connect('data/databases/bg3_cloaks.db')
c = conn.cursor()
...
cloaks_url = "https://bg3.wiki/wiki/Cloaks"

# Requête pour obtenir le contenu de la page
response = requests.get(cloaks_url)
if response.status_code == 200:
    ...
    for a in soup.select('table.wikitable a'):
        ...
        if a['href'].startswith('/wiki/') and img and img.get('width') == '50' and img.get('height') == '50':
            cloaks_links.append(base_url + a['href'])

    # Itérer sur chaque lien de vêtement pour scraper les données
    for cloaks_url in cloaks_links:
        response = requests.get(cloaks_url)
        if response.status_code == 200:
            cloaks_soup = BeautifulSoup(response.content, 'html.parser')

            # Extraction des données
            name = cloaks_soup.find('h1', id='firstHeading').get_text(strip=True, separator=" ")

            # Vérification si l'élément quote existe avant d'appeler get_text()
            description_element_tag = cloaks_soup.find('meta', property='og:description')
            description = description_element_tag["content"] if description_element_tag else ""

            # Vérification si l'élément quote existe avant d'appeler get_text()
            quote_element = cloaks_soup.find('div', class_='bg3wiki-blockquote-text')
            ...
            script_content = cloaks_soup.find('script').string
            ...
            if match:
                ...
                cloak_type = wg_categories[0] if wg_categories else None
            else:
                print(f"wgCategories not found for {name}.")
                cloak_type = None

            # Télécharger l'image principale (floatright)
            main_image = cloaks_soup.find('img', alt=name + ' image')
            ...
            # Rareté
            rarity_element = find_li_by_text(cloaks_soup, 'Rarity:')
            rarity = rarity_element.get_text(strip=True, separator=" ").split(':')[
                -1].strip() if rarity_element else 'Unknown'

            # Poids
            weight_element = find_li_by_text(cloaks_soup, 'Weight:')
            if weight_element:
            ...
            # Traitement du prix
            price_element = find_li_by_text(cloaks_soup, 'Price:')
            if price_element:
            ...
            # UID et UUID
            uid_element = cloaks_soup.find_all('tt')
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
            uuid, name, description, quote, cloak_type, rarity, weight_kg, weight_lb, price_gp,
            uid, image_path))

            # Extraction des effets spéciaux des <dl>, <ul> et autres éléments après "Special"
            specials_section = cloaks_soup.find('h3', string="Special")
            if specials_section:
            ...
            location_section = cloaks_soup.find('h2', string=lambda x: x and "Where to find" in x)
            if location_section:
