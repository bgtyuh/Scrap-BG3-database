...
image_folder = 'data/images/amulet_images'
...
conn = sqlite3.connect('data/databases/bg3_amulets.db')
c = conn.cursor()
...
amulets_url = "https://bg3.wiki/wiki/Amulets"

# Requête pour obtenir le contenu de la page
response = requests.get(amulets_url)
if response.status_code == 200:
    ...
    for a in soup.select('table.wikitable a'):
        ...
        if a['href'].startswith('/wiki/') and img and img.get('width') == '50' and img.get('height') == '50':
            amulets_links.append(base_url + a['href'])

    # Itérer sur chaque lien de vêtement pour scraper les données
    for amulets_url in amulets_links:
        response = requests.get(amulets_url)
        if response.status_code == 200:
            amulets_soup = BeautifulSoup(response.content, 'html.parser')

            # Extraction des données
            name = amulets_soup.find('h1', id='firstHeading').get_text(strip=True, separator=" ")

            # Vérification si l'élément quote existe avant d'appeler get_text()
            description_element_tag = amulets_soup.find('meta', property='og:description')
            description = description_element_tag["content"] if description_element_tag else ""

            # Vérification si l'élément quote existe avant d'appeler get_text()
            quote_element = amulets_soup.find('div', class_='bg3wiki-blockquote-text')
            ...
            script_content = amulets_soup.find('script').string
            ...
            if match:
                ...
                wg_categories.remove('Pages using duplicate arguments in template calls') if wg_categories and 'Pages using duplicate arguments in template calls' in wg_categories else None
                wg_categories.remove('Misc pages without images') if wg_categories and 'Misc pages without images' in wg_categories else None
                wg_categories.remove('Item pages without controller icons') if wg_categories and 'Item pages without controller icons' in wg_categories else None
                amulet_type = wg_categories[0] if wg_categories else None
            else:
                print(f"wgCategories not found for {name}.")
                amulet_type = None

            # Télécharger l'image principale (floatright)
            main_image = amulets_soup.find('img', alt=name + ' image')
            ...
            # Rareté
            rarity_element = find_li_by_text(amulets_soup, 'Rarity:')
            rarity = rarity_element.get_text(strip=True, separator=" ").split(':')[
                -1].strip() if rarity_element else 'Unknown'

            # Poids
            weight_element = find_li_by_text(amulets_soup, 'Weight:')
            if weight_element:
            ...
            # Traitement du prix
            price_element = find_li_by_text(amulets_soup, 'Price:')
            if price_element:
            ...
            # UID et UUID
            uid_element = amulets_soup.find_all('tt')
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
            uuid, name, description, quote, amulet_type, rarity, weight_kg, weight_lb, price_gp,
            uid, image_path))

            # Extraction des effets spéciaux des <dl>, <ul> et autres éléments après "Special"
            specials_section = amulets_soup.find('h3', string="Special")
            if specials_section:
            ...
            location_section = amulets_soup.find('h2', string=lambda x: x and "Where to find" in x)
            if location_section:
