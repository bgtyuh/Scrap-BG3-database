...
image_folder = 'data/images/armour_images'
...
conn = sqlite3.connect('data/databases/bg3_armours.db')
c = conn.cursor()
...
armours_url = "https://bg3.wiki/wiki/Armour"

# Requête pour obtenir le contenu de la page
response = requests.get(armours_url)
if response.status_code == 200:
    ...
    for a in soup.select('table.wikitable a'):
        ...
        if a['href'].startswith('/wiki/') and img and img.get('width') == '50' and img.get('height') == '50':
            armours_links.append(base_url + a['href'])

    # Itérer sur chaque lien de vêtement pour scraper les données
    for armours_url in armours_links:
        response = requests.get(armours_url)
        if response.status_code == 200:
            armours_soup = BeautifulSoup(response.content, 'html.parser')

            # Extraction des données
            name = armours_soup.find('h1', id='firstHeading').get_text(strip=True, separator=" ")

            # Vérification si l'élément quote existe avant d'appeler get_text()
            description_element_tag = armours_soup.find('meta', property='og:description')
            description = description_element_tag["content"] if description_element_tag else ""

            # Vérification si l'élément quote existe avant d'appeler get_text()
            quote_element = armours_soup.find('div', class_='bg3wiki-blockquote-text')
            quote = quote_element.get_text(strip=True, separator=" ") if quote_element else ""

            # Télécharger l'image principale (floatright)
            main_image = armours_soup.find('img', alt=name + ' image')
            ...
            # Poids
            weight_element = find_li_by_text(armours_soup, 'Weight:')
            if weight_element:
            ...
            # Traitement du prix
            price_element = find_li_by_text(armours_soup, 'Price:')
            if price_element:
            ...
            # Classe d'armure
            armour_class_base_element = armours_soup.find('div', class_='ac-value')
            armour_class_base = int(
                armour_class_base_element.get_text(strip=True, separator=" ")) if armour_class_base_element else 0

            armour_class_modifier_element = armours_soup.find('div', class_='ac-value-comment')
            ...
            # UID et UUID
            uid_element = armours_soup.find_all('tt')
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
            uuid, name, description, quote, armour_type, rarity, weight_kg, weight_lb, price_gp, armour_class_base, armour_class_modifier,
            uid, image_path))

            # Extraction des effets spéciaux des <dl>, <ul> et autres éléments après "Special"
            specials_section = armours_soup.find('h3', string="Special")
            if specials_section:
            ...
            location_section = armours_soup.find('h2', string=lambda x: x and "Where to find" in x)
            if location_section:
