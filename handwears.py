...
handwears_url = "https://bg3.wiki/wiki/Handwear"

# Requête pour obtenir le contenu de la page
response = requests.get(handwears_url)
if response.status_code == 200:
    ...
    for a in soup.select('table.wikitable a'):
        ...
        if a['href'].startswith('/wiki/') and img and img.get('width') == '50' and img.get('height') == '50':
            handwears_links.append(base_url + a['href'])

    # Itérer sur chaque lien de vêtement pour scraper les données
    for handwears_url in handwears_links:
        response = requests.get(handwears_url)
        if response.status_code == 200:
            handwears_soup = BeautifulSoup(response.content, 'html.parser')

            # Extraction des données
            name = handwears_soup.find('h1', id='firstHeading').get_text(strip=True, separator=" ")

            # Vérification si l'élément quote existe avant d'appeler get_text()
            description_element_tag = handwears_soup.find('meta', property='og:description')
            description = description_element_tag["content"] if description_element_tag else ""

            # Vérification si l'élément quote existe avant d'appeler get_text()
            quote_element = handwears_soup.find('div', class_='bg3wiki-blockquote-text')
            ...
            script_content = handwears_soup.find('script').string
            ...
            if match:
                ...
                wg_categories.remove('Damage Rider As Source') if wg_categories and 'Damage Rider As Source' in wg_categories else None
                wg_categories.remove('Bugs') if wg_categories and 'Bugs' in wg_categories else None
                handwear_type = wg_categories[0] if wg_categories else None
            else:
                print(f"wgCategories not found for {name}.")
                handwear_type = None

            # Télécharger l'image principale (floatright)
            main_image = handwears_soup.find('img', alt=name + ' image')
            ...
            # Rareté
            rarity_element = find_li_by_text(handwears_soup, 'Rarity:')
            rarity = rarity_element.get_text(strip=True, separator=" ").split(':')[
                -1].strip() if rarity_element else 'Unknown'

            # Poids
            weight_element = find_li_by_text(handwears_soup, 'Weight:')
            if weight_element:
            ...
            # Traitement du prix
            price_element = find_li_by_text(handwears_soup, 'Price:')
            if price_element:
            ...
            # UID et UUID
            uid_element = handwears_soup.find_all('tt')
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
            uuid, name, description, quote, handwear_type, rarity, weight_kg, weight_lb, price_gp,
            uid, image_path))

            # Extraction des effets spéciaux des <dl>, <ul> et autres éléments après "Special"
            specials_section = handwears_soup.find('h3', string="Special")
            if specials_section:
            ...
            location_section = handwears_soup.find('h2', string=lambda x: x and "Where to find" in x)
            if location_section:
